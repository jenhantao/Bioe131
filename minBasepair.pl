...
$nucAdded =0;
	...
	if (($numA+$numU+$numC+$numG)>0) {
		...
		$numA--;
		$nucAdded = 1;
		}elsif($rand == 1 && $caseU <= $caseA && $caseU <= $caseG && $caseU <= $caseU && $numU>0&& $nucAdded==0) {
		$sequence = $sequence."U";
		$numU--;
		$nucAdded = 1;
		} elsif($rand == 2 && $caseC <= $caseU && $caseC <= $caseG && $caseC <= $caseA && $numC>0&& $nucAdded==0) {
		$sequence = $sequence."C";
		$numC--;
		$nucAdded = 1;
		} elsif($rand == 3 && $caseG <= $caseU && $caseG <= $caseA && $caseG <= $caseC && $numG>0&& $nucAdded==0) {
		$sequence = $sequence."G";
		$numG--;
		$nucAdded = 1;
		} elsif($nucAdded ==0) {
